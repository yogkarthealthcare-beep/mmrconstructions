...
export class ProfileComponent implements OnInit {
  loading = true; editMode = false; saving = false;
  profile: any = {}; toast = ''; toastType = 'success';

  editable = { email: '', alternate_mobile: '', spouse_name: '', bank_name: '', account_holder_name: '', branch_name: '', nominee_name: '', nominee_relationship: '' };
  ...
  save() {
    ...
    this.api.updateProfile(this.editable).subscribe({
      next: (res: any) => {
        this.showToast(res.message || 'Profile updated!', 'success');
        this.editMode = false; this.saving = false;
        this.ngOnInit();
      },
      error: (e: any) => { this.showToast(e?.error?.message || 'Update failed', 'error'); this.saving = false; }
    });
  }
